...
function extractHistoryText(message) {
...
function isThouSessionKeyForConversation(sessionKey, conversationId) {
...
function isThouDashboardSessionKeyForConversation(sessionKey, conversationId) {
    const normalizedKey = normalizeSessionKey(sessionKey);
    const normalizedConversationId = normalizeSessionKey(conversationId);
    if (!normalizedKey || !normalizedConversationId) {
        return false;
...
function createThouMainSessionKey(agentId) {
    return `agent:${normalizeAgentId(agentId)}:main`;
}
function createThouDashboardSessionKey(agentId) {
    return `agent:${normalizeAgentId(agentId)}:dashboard:${crypto.randomUUID()}`;
}
async function withGatewayClient(run) {
...
export async function listThouGatewayAgents() {
    return await withGatewayClient(async (client) => {
        const response = (await client.request("agents.list", {}, 15000));
    ...
    return await withGatewayClient(async (client) => {
        ...
        const normalized = rawSessions
            ...
            const label = (typeof session.label === "string" && session.label.trim()) ||
                ...
                key;
            const preview = typeof session.lastMessagePreview === "string" && session.lastMessagePreview.trim()
...
export async function createThouGatewaySession(params) {
    ...
    return await withGatewayClient(async (client) => {
        const response = (await client.request("sessions.create", {
            agentId,
            key: requestedKey,
            label: requestedLabel,
        }, 15000));
        const key = typeof response?.key === "string" && response.key.trim() ? response.key.trim() : requestedKey;
        const label = (typeof response?.entry?.label === "string" && response.entry.label.trim()) || requestedLabel;
        return {
            key,
            label,
            updatedAt: Date.now(),
...
export async function loadThouGatewayHistory(params) {
    const sessionKey = String(params.sessionKey ?? "").trim() || createThouMainSessionKey(normalizeAgentId(params.agentId));
    if (!sessionKey) {
        return [];
    }
...
function deriveDeviceIdentityFromPublicKey(publicKeyPem) {
...
function getGatewayIdentityFile() {
...
function ensureGatewayDeviceIdentity() {
    ...
    if (fs.existsSync(identityFile)) {
        try {
            const stored = JSON.parse(fs.readFileSync(identityFile, "utf8"));
...
function createGatewayRuntime() {
    ...
    const token = process.env.OPENCLAW_GATEWAY_TOKEN ?? config?.gateway?.auth?.token ?? config?.auth?.token ?? "";
    if (!token.trim()) {
...
class GatewayRpcClient {
    ...
    resolveConnect = null;
    rejectConnect = null;
    deviceIdentity = ensureGatewayDeviceIdentity();
    constructor(options) {
    ...
    handleMessage(raw) {
        ...
        const errorShape = (message.error ?? {});
...
export function createThouGatewayChatRun(params) {
    const targetSessionKey = String(params.sessionKey ?? "").trim() || createThouMainSessionKey(normalizeAgentId(params.agentId));
    const gateway = createGatewayRuntime();
    const client = new GatewayRpcClient({
        url: gateway.wsUrl,
        token: gateway.token,
    ...
    const finishStream = async (finalize) => {
        if (streamFinished) {
            return;
        }
        streamFinished = true;
    ...
    const completion = (async () => {
        try {
            ...
            const nextError = disconnectError ?? (error instanceof Error ? error : new Error(String(error)));
            await client.stop();
            if (!abortRequested) {
