...
import { createThouGatewayChatRun, createThouGatewaySession, listThouGatewayAgents, listThouGatewaySessions, loadThouGatewayHistory, } from "./bridge-chat-runtime.js";
...
function collectMobileHostHints(listenHost) {
    const hints = new Set();
...
function appendHostHint(candidate, hints) {
...
export async function startThouBridgeForAccount(params) {
    params.log?.info?.(`[${params.account.accountId}] resolving Thou bridge auth token in plugin state dir`);
    const { token, tokenFile } = ensureThouBridgeAuthToken({
        accountId: params.account.accountId,
    });
    params.log?.info?.(`[${params.account.accountId}] Thou bridge auth token ready (${tokenFile})`);
    params.statusSink({
        running: true,
        connected: false,
    ...
    const server = await ThouBridgeServer.listen({
        ...
        getMobileConnectionProfile: () => getThouMobileConnectionProfile({
            account: params.account,
        }),
        createInboundChatRun: ({ deviceId, text, agentId, sessionKey, onStart, onChunk, onDone, onError }) => createThouGatewayChatRun({
            text,
        ...
        onDisconnected: ({ deviceId, code, reason }) => {
            ...
            params.log?.warn?.(`[${params.account.accountId}] Thou device disconnected (${deviceId}, code=${code}${reasonText ? `, reason=${reasonText}` : ""})`);
            params.statusSink({
                connected: server.connectionCount > 0,
                lastDisconnect: {
                    at: Date.now(),
        ...
        onAuthFailed: ({ deviceId, deviceLabel, providedToken }) => {
            ...
            params.log?.warn?.(`[${params.account.accountId}] Thou auth failed (deviceId=${deviceId ?? "unknown"}, deviceLabel=${deviceLabel ?? "unknown"}, token=${tokenHint})`);
            params.statusSink({
    ...
    params.log?.info?.(`[${params.account.accountId}] Thou bridge listen completed (${params.account.transport.listenHost}:${params.account.transport.listenPort}${params.account.transport.listenPath})`);
    registerThouBridge({
        accountId: params.account.accountId,
        server,
        authToken: token,
    ...
    params.log?.info?.(`[${params.account.accountId}] Thou bridge listening on ${server.url} (token file: ${tokenFile})`);
    return {
        stop: async () => {
