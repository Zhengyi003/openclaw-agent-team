...
import { THOU_PAIRING_APPROVED_MESSAGE, THOU_DEFAULT_DM_POLICY, } from "./types.js";
...
function normalizeThouMessagingTarget(raw) {
...
function looksLikeThouTargetId(raw, normalized) {
...
const thouChatPlugin = createChatChannelPlugin({
    ...
    security: {
        ...
        collectWarnings: ({ cfg, accountId }) => collectThouChannelWarnings({
            cfg: cfg,
            accountId,
        }),
    },
    ...
    outbound: {
        ...
        attachedResults: {
            channel: "thou",
            sendText: async ({ to, text, accountId, replyToId }) => await deliverThouText({
                to,
                text,
                accountId,
                replyToId,
            }),
            sendMedia: async ({ to, text, mediaUrl, accountId, replyToId }) => await deliverThouMedia({
                to,
                text,
                mediaUrl,
                accountId,
        ...
        buildAccountSnapshot: async ({ account, runtime }) => {
            ...
            return {
                ...
                connectionSetup: connectionPayload
                    ? {
                        instruction: "在 iPhone 端打开 Thou -> OpenClaw -> 连接我的 Mac，然后粘贴下面这整行连接信息。",
                        preferredHost: connectionPayload.preferredHost,
                        alternateHosts: connectionPayload.hosts.filter((host) => host !== connectionPayload.preferredHost),
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            const statusSink = (patch) => {
                ctx.log?.info?.(`[${account.accountId}] Thou runtime patch ${JSON.stringify(patch)}`);
                writeStatus(patch);
            };
            if (!account.enabled || account.transport.mode !== "sidecar-bridge") {
                ctx.log?.info(`[${account.accountId}] Thou bridge skipped (enabled=${String(account.enabled)}, mode=${account.transport.mode})`);
                statusSink({
                    running: false,
                    connected: false,
                    mode: account.transport.mode,
            ...
            ctx.log?.info(`[${account.accountId}] starting Thou bridge (${account.transport.listenHost}:${account.transport.listenPort}${account.transport.listenPath})`);
            await runPassiveAccountLifecycle({
                abortSignal: ctx.abortSignal,
                start: async () => await startThouBridgeForAccount({
                    account,
                    statusSink,
                    log: ctx.log,
                }),
