...
const TEMPLATES: AgentWorkTemplate[] = [
  {
    id: "coding",
    label: "Coding",
...
let guiState: AgentWorkGuiState = {
  account: null,
...
async function buildConnectSummary(guiUrl: string): Promise<AgentWorkConnectSummary> {
  if (!guiState.account) {
    return {
      ready: false,
...
async function buildAgentPreview(payload: CreateAgentRequest): Promise<AgentPreviewResult> {
...
function humanizeAgentCreateError(errorMessage: string, preview: AgentPreviewResult): string {
...
function humanizeAgentDismissError(agentId: string, errorMessage: string): string {
  const normalized = errorMessage.trim();
  const lower = normalized.toLowerCase();
...
async function dismissAgent(agentId: string): Promise<void> {
  try {
    await execFileAsync(
      "openclaw",
...
async function listAgents(): Promise<AgentRosterEntry[]> {
  try {
...
function buildConnectionCacheKey(account: ResolvedThouAccount | null): string | null {
  if (!account) {
    return null;
  }
...
function haveConnectArtifactsExpired(): boolean {
...
function renderAgentWorkShellHtml(): string {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
