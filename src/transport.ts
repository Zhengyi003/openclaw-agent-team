...
export function createThouNotImplementedError(scope: string): Error {
  return new Error(
    `OpenClaw Agent Team v1 only defines ownership boundaries for ${scope}; live transport still stays on the sidecar reference path.`,
  );
}
