...
import type { ThouRootConfig } from "../channel/types.js";

export const thouSetupWizard: ChannelSetupWizard = {
	channel: "thou",
	status: {
		...
		resolveSelectionHint: ({ configured }) =>
			configured ? "复制连接信息" : "先完成 Thou 配置",
		resolveConfigured: ({ cfg }) => {
			const accountId = resolveDefaultThouAccountId(cfg as ThouRootConfig);
			return resolveThouAccount({ cfg: cfg as ThouRootConfig, accountId }).configured;
		},
		resolveStatusLines: async ({ cfg, configured }) => {
			const accountId = resolveDefaultThouAccountId(cfg as ThouRootConfig);
			const account = resolveThouAccount({ cfg: cfg as ThouRootConfig, accountId });
			...
			const alternateHosts = connectionPayload.hosts.filter(
